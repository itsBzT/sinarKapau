...
protected $column_order = array(null,'kode','namaproduk','katnama','stok_tersedia','harga_jual',null);
protected $column_search = array('kode','namaproduk');
...
private function _get_datatables_query($keywordkode)
{
    if(strlen($keywordkode) == 0){
        ...
        $this->dt = $this->db->table($this->table)->join('kategori','katid=produk_katid')->like('kode',$keywordkode)->orLike('namaproduk',$keywordkode);
    }
...
public function count_all($keywordkode)
{
    if(strlen($keywordkode)==0){
        ...
        $tbl_storage = $this->db->table($this->table)->join('kategori','katid=produk_katid')->like('kode',$keywordkode)->orLike('namaproduk',$keywordkode);
    }
